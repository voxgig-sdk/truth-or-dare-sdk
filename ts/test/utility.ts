...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.TRUTHORDARE_TEST_LIVE ||
    'TRUE' === process.env.TRUTHORDARE_TEST_OVERRIDE
  ) {
  ...
  m.TRUTHORDARE_TEST_EXPLAIN = process.env.TRUTHORDARE_TEST_EXPLAIN || m.TRUTHORDARE_TEST_EXPLAIN
