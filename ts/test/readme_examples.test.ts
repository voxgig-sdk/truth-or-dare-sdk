...
//     (\`new TruthOrDareSDK(...)\` / \`TruthOrDareSDK.test(...)\` become
//     \`TruthOrDareSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"dare":{"test01":{"id":"test01"}},"nhie":{"test01":{"id":"test01"}},"paranoia":{"test01":{"id":"test01"}},"truth":{"test01":{"id":"test01"}},"wyr":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TruthOrDareSDK(...)\` and any
// \`TruthOrDareSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TruthOrDareSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TruthOrDareSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
