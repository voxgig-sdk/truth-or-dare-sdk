...
module TruthOrDareUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TruthOrDareResponse.new({})
    ...
    if fetch_err
      ...
      response = TruthOrDareResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TruthOrDareResponse.new(fetched)
    else
