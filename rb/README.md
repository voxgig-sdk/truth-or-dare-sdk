# TruthOrDare Ruby SDK

The Ruby SDK for the TruthOrDare API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Dare(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.TruthOrDare(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── TruthOrDare_sdk.rb       -- Main SDK module
