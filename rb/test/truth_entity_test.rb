...
class TruthEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = truth_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "truth." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    truth_ref01_data = nil
    if truth_ref01_data_raw.length > 0
      truth_ref01_data = Helpers.to_map(truth_ref01_data_raw[0][1])
    end

    # LOAD
    truth_ref01_ent = client.Truth(nil)
...
def truth_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "truth", "TruthTestData.json")
