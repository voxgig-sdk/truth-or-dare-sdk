...
class NhieEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TRUTH_OR_DARE_TEST_NHIE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    nhie_ref01_data_dt0_loaded = nhie_ref01_ent.load(nhie_ref01_match_dt0, nil)
    nhie_ref01_data_dt0_load_result = Helpers.to_map(nhie_ref01_data_dt0_loaded.respond_to?(:data_get) ? nhie_ref01_data_dt0_loaded.data_get : nhie_ref01_data_dt0_loaded)
...
def nhie_basic_setup(extra)
  ...
  entid_env_raw = ENV["TRUTH_OR_DARE_TEST_NHIE_ENTID"]
