...
class ParanoiaEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TRUTH_OR_DARE_TEST_PARANOIA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    paranoia_ref01_data_dt0_load_result = Helpers.to_map(paranoia_ref01_data_dt0_loaded.respond_to?(:data_get) ? paranoia_ref01_data_dt0_loaded.data_get : paranoia_ref01_data_dt0_loaded)
...
def paranoia_basic_setup(extra)
  ...
  entid_env_raw = ENV["TRUTH_OR_DARE_TEST_PARANOIA_ENTID"]
