...
class ParanoiaEntity
  ...
  def make
    opts = @_entopts.dup
    ParanoiaEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TruthOrDareHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Paranoia, Hash] the current Paranoia data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TruthOrDareHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Paranoia fields)
  def match_get
  ...
  #
  # @param reqmatch [ParanoiaLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Paranoia.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Paranoia, Hash] the loaded Paranoia; raises TruthOrDareError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TruthOrDareHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
