...
class FtCtrl extends TruthOrDareControl
{
...
class FtClient
{
    ...
    private TruthOrDareUtility $utility;

    public function __construct(TruthOrDareUtility $utility, array $options)
    {
    ...
    public function get_utility(): TruthOrDareUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TruthOrDareContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TruthOrDareContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TruthOrDareContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TruthOrDareConfig::make_feature($name);
    ...
    private static function buildUrl(TruthOrDareSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TruthOrDareError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TruthOrDareResponse($fetched);
        }
    ...
    private function fail(TruthOrDareContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TruthOrDareContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
