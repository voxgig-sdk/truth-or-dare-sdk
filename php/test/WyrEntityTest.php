...
class WyrEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TruthOrDareSDK::test(null, null);
        $ent = $testsdk->Wyr(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = wyr_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "wyr." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $wyr_ref01_data = null;
        if (count($wyr_ref01_data_raw) > 0) {
            $wyr_ref01_data = Helpers::to_map($wyr_ref01_data_raw[0][1]);
        }

        // LOAD
        $wyr_ref01_ent = $client->Wyr(null);
        $wyr_ref01_match_dt0 = [
            "id" => $wyr_ref01_data["id"],
        ];
        [$wyr_ref01_data_dt0_loaded, $err] = $wyr_ref01_ent->load($wyr_ref01_match_dt0, null);
        $this->assertNull($err);
...
function wyr_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/wyr/WyrTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["wyr01", "wyr02", "wyr03"] as $k) {
