...
// TruthOrDare SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TruthOrDareOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TruthOrDareOmni::structfix($now, Struct::undef()))
                        !== json_encode(TruthOrDareOmni::structfix($val, Struct::undef()))) {
