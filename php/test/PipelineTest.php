...
{
    private static function utility(): TruthOrDareUtility
    {
        return new TruthOrDareUtility();
    }
    ...
    private static function ctx(array $over = []): TruthOrDareContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TruthOrDareContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TruthOrDareOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TruthOrDareError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TruthOrDareOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TruthOrDareMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TruthOrDareOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TruthOrDareMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TruthOrDareOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TruthOrDareMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TruthOrDareSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TruthOrDareMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TruthOrDareResponse([]),
            'result' => new TruthOrDareResult([]),
        ]);
        [, $err] = TruthOrDareMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TruthOrDareSpec([]),
            'response' => null,
            'result' => new TruthOrDareResult([]),
        ]);
        [, $err] = TruthOrDareMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TruthOrDareSpec(['step' => 's']),
            'response' => new TruthOrDareResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TruthOrDareResult([]),
        ]);
        [, $err] = TruthOrDareMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TruthOrDareSpec(['step' => 's']),
            'response' => new TruthOrDareResponse(self::resp(200, ['v' => 1])),
            'result' => new TruthOrDareResult([]),
        ]);
        [, $err] = TruthOrDareMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TruthOrDareSpec(['step' => 's']),
            'response' => new TruthOrDareResponse(self::resp(200, ['v' => 2])),
            'result' => new TruthOrDareResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TruthOrDareMakeResponse::call($ctx);
    ...
    {
        $preset = new TruthOrDareResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TruthOrDareSpec([]),
            'response' => new TruthOrDareResponse([]),
            'result' => new TruthOrDareResult([]),
        ...
        [$got, $err] = TruthOrDareMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TruthOrDareResult([])]);
        [, $err] = TruthOrDareMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TruthOrDareSpec([]), 'result' => null]);
        [, $err] = TruthOrDareMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TruthOrDareSpec(['step' => 's']),
            'result' => new TruthOrDareResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TruthOrDareOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TruthOrDareMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TruthOrDareSpec(['step' => 's']),
            'result' => new TruthOrDareResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TruthOrDareOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TruthOrDareMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TruthOrDareResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TruthOrDareSpec([]),
            'result' => new TruthOrDareResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TruthOrDareMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TruthOrDareMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TruthOrDareError('boom', 'boom');
        $utility->fetcher = function (TruthOrDareContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TruthOrDareSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TruthOrDareMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TruthOrDareContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TruthOrDareMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TruthOrDareResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        TruthOrDareMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TruthOrDareContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TruthOrDareSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TruthOrDareMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TruthOrDareResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TruthOrDareSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TruthOrDareMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TruthOrDareMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TruthOrDareMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TruthOrDareResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TruthOrDareDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TruthOrDareResult(['ok' => false])]);
        $this->expectException(TruthOrDareError::class);
        TruthOrDareDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TruthOrDareResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TruthOrDareMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TruthOrDareMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TruthOrDareBaseFeature();
        $b = new TruthOrDareBaseFeature();
        TruthOrDareFeatureAdd::call($ctx, $a);
        TruthOrDareFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TruthOrDareBaseFeature
    {
        $f = new TruthOrDareBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TruthOrDareFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TruthOrDareContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TruthOrDareSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TruthOrDarePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        TruthOrDarePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TruthOrDareResponse(['headers' => null]),
            'result' => new TruthOrDareResult([]),
        ]);
        TruthOrDareResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        TruthOrDareResultBody::call($ctx);
