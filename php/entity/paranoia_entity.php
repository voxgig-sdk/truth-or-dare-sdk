...
// TruthOrDare SDK Paranoia entity
...
class ParanoiaEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ParanoiaEntity($this->_client, $opts);
    }

    /**
     * @param Paranoia|array $args Paranoia data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TruthOrDareHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Paranoia|array The current Paranoia data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Paranoia fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TruthOrDareHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Paranoia fields).
     */
    ...
    /**
     * Load a single Paranoia.
     *
     * @param ParanoiaLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ParanoiaLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Paranoia|array The loaded Paranoia as an assoc-array at the
     *   SDK boundary; throws TruthOrDareError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TruthOrDareHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
