...
class TruthOrDareMakeError
{
    public static function call(?TruthOrDareContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TruthOrDareContext([], null);
        }
        $op = $ctx->op ?? new TruthOrDareOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TruthOrDareResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TruthOrDareError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TruthOrDareError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof TruthOrDareError) {
