...
type Args struct {
	Entity string         `json:"entity" jsonschema:"dare | nhie | paranoia | truth | wyr"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.TruthOrDareSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "truth-or-dare_list",
		Description: "List records from TruthOrDare. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "truth-or-dare_load",
		Description: "Load a single record from TruthOrDare. " +
...
func runOp(client *sdk.TruthOrDareSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.TruthOrDareSDK, name string) (sdk.TruthOrDareEntity, error) {
	switch strings.ToLower(name) {
	case "dare":
		return client.Dare(nil), nil
	case "nhie":
		return client.Nhie(nil), nil
	case "paranoia":
		return client.Paranoia(nil), nil
	case "truth":
		return client.Truth(nil), nil
	case "wyr":
		return client.Wyr(nil), nil
