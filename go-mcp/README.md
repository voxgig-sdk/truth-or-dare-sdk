...
[MCP](https://modelcontextprotocol.io) server exposing the TruthOrDare SDK as
two agent tools — `truth-or-dare_list` and `truth-or-dare_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/truth-or-dare-mcp)
make build

# 2. Provide credentials via the environment
export TRUTH_OR_DARE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user truth-or-dare \
  -- /absolute/path/to/truth-or-dare-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./truth-or-dare-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// truth-or-dare_list: first page of records
{ "entity": "dare" }
{ "entity": "dare", "query": { } }

// truth-or-dare_load: one record by id
{ "entity": "dare", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/truth-or-dare-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user truth-or-dare \
     -- "$PWD"/dist/*/truth-or-dare-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `truth-or-dare_list` and `truth-or-dare_load` tools now appear
   in new sessions. Ask the agent to *"list dare using truth-or-dare"*
   and it calls `truth-or-dare_list` with `{"entity":"dare"}`.
...
```sh
export TRUTH_OR_DARE_APIKEY=sk_live_xxx            # API key
export TRUTH_OR_DARE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `truth-or-dare_list` tool
...
### Call the `truth-or-dare_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `truth-or-dare_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `truth-or-dare_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TRUTH_OR_DARE_APIKEY` | API key sent with every request. |
| `TRUTH_OR_DARE_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

dare | nhie | paranoia | truth | wyr

### Smoke test via HTTP (raw JSON-RPC)

```sh
./truth-or-dare-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"truth-or-dare_load","arguments":{"entity":"dare","query":{"id":1}}}}'
```
