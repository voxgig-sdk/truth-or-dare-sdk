...
boru-driven command-line client **and** interactive REPL for the TruthOrDare
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/truth-or-dare-cli)
make build

# 2. See usage (words, entities, env vars)
./truth-or-dare-cli --help

# 3. Provide credentials once, via the environment
export TRUTH_OR_DARE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./truth-or-dare-cli load 1 dare            # {id:1} shorthand
./truth-or-dare-cli load '{id:1}' dare       # explicit match map

# 5. Override the API base URL for a single call
TRUTH_OR_DARE_BASE=https://api.example.com ./truth-or-dare-cli load 1 dare

# 6. No arguments -> interactive REPL
./truth-or-dare-cli
truth-or-dare> load 1 dare
truth-or-dare> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/truth-or-dare-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/truth-or-dare-cli load 1 dare
   ```
...
```sh
./truth-or-dare-cli load 1 dare          # scalar shorthand for {id:1}
./truth-or-dare-cli load '{id:1}' dare     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `truth-or-dare>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `dare`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/truth-or-dare-cli`. |
...
(`./truth-or-dare-cli <expr>`) and interactively (the REPL), and expressions compose the
