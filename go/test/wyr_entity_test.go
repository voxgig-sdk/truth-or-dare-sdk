...
func TestWyrEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TRUTH_OR_DARE_TEST_WYR_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		wyrRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.wyr")))
		...
		wyrRef01DataDt0LoadResult := core.ToMapAny(entityData(wyrRef01DataDt0Loaded))
...
func wyrBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TRUTH_OR_DARE_TEST_WYR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TRUTH_OR_DARE_TEST_WYR_ENTID"])
