...
import (
	...
	omni "github.com/voxgig-sdk/truth-or-dare-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TruthOrDareError); is {
				out := map[string]any{
					"name":    "TruthOrDareError",
...
func omniCtx(arg any, client *sdk.TruthOrDareSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TruthOrDareSDK); is {
			client = live
