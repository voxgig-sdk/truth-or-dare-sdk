...
The Lua SDK for the TruthOrDare API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Dare(nil):load({ id = "example_id" }, nil)
...
local result, err = client:TruthOrDare(nil):load(
  { id = "test01" }, nil
)
