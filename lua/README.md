...
The Lua SDK for the TruthOrDare API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Dare()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/truth-or-dare-sdk/releases)),
...
```
lua/
├── truth-or-dare_sdk.lua    -- Main SDK module
...
The main module (`truth-or-dare_sdk`) exports the SDK constructor
