...
local WyrEntity = {}
WyrEntity.__index = WyrEntity


function WyrEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WyrEntity)
  self._name = "wyr"
  self._client = client
...
function WyrEntity:get_name()
  return self._name
end


function WyrEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WyrEntity.new(self._client, opts)
end
...
function WyrEntity:mark_deleted()
  self._deleted = true
end


function WyrEntity:deleted()
  return true == self._deleted
end


function WyrEntity:data_set(args)
  if args ~= nil then
...
function WyrEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WyrEntity:match_set(args)
  if args ~= nil then
...
function WyrEntity:stream(action, args, callopts)
...
---@param reqmatch WyrLoadMatch
---@param ctrl? table
---@return Wyr
---@return string? err
function WyrEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WyrEntity:_run_op(ctx, post_done)
  local utility = self._utility
