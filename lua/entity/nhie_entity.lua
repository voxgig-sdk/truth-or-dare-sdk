...
local NhieEntity = {}
NhieEntity.__index = NhieEntity


function NhieEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NhieEntity)
  self._name = "nhie"
  self._client = client
...
function NhieEntity:get_name()
  return self._name
end


function NhieEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NhieEntity.new(self._client, opts)
end
...
function NhieEntity:mark_deleted()
  self._deleted = true
end


function NhieEntity:deleted()
  return true == self._deleted
end


function NhieEntity:data_set(args)
  if args ~= nil then
...
function NhieEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NhieEntity:match_set(args)
  if args ~= nil then
...
function NhieEntity:stream(action, args, callopts)
...
---@param reqmatch NhieLoadMatch
---@param ctrl? table
---@return Nhie
---@return string? err
function NhieEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NhieEntity:_run_op(ctx, post_done)
  local utility = self._utility
