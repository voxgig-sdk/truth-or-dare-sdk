...
local TruthEntity = {}
TruthEntity.__index = TruthEntity


function TruthEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TruthEntity)
  self._name = "truth"
  self._client = client
...
function TruthEntity:get_name()
  return self._name
end


function TruthEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TruthEntity.new(self._client, opts)
end
...
function TruthEntity:mark_deleted()
  self._deleted = true
end


function TruthEntity:deleted()
  return true == self._deleted
end


function TruthEntity:data_set(args)
  if args ~= nil then
...
function TruthEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TruthEntity:match_set(args)
  if args ~= nil then
...
function TruthEntity:match_get()
...
function TruthEntity:stream(action, args, callopts)
...
function TruthEntity:_run_op(ctx, post_done)
  local utility = self._utility
