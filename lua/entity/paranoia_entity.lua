...
local ParanoiaEntity = {}
ParanoiaEntity.__index = ParanoiaEntity


function ParanoiaEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ParanoiaEntity)
  self._name = "paranoia"
  self._client = client
...
function ParanoiaEntity:get_name()
  return self._name
end


function ParanoiaEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ParanoiaEntity.new(self._client, opts)
end
...
function ParanoiaEntity:mark_deleted()
  self._deleted = true
end


function ParanoiaEntity:deleted()
  return true == self._deleted
end


function ParanoiaEntity:data_set(args)
  if args ~= nil then
...
function ParanoiaEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ParanoiaEntity:match_set(args)
  if args ~= nil then
...
function ParanoiaEntity:match_get()
...
function ParanoiaEntity:stream(action, args, callopts)
...
---@param reqmatch ParanoiaLoadMatch
---@param ctrl? table
---@return Paranoia
---@return string? err
function ParanoiaEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ParanoiaEntity:_run_op(ctx, post_done)
  local utility = self._utility
