...
describe("DareEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRUTH_OR_DARE_TEST_DARE_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local dare_ref01_data_dt0_load_result = helpers.to_map(type(dare_ref01_data_dt0_loaded) == 'table' and dare_ref01_data_dt0_loaded.data_get and dare_ref01_data_dt0_loaded:data_get() or dare_ref01_data_dt0_loaded)
...
function dare_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("TRUTH_OR_DARE_TEST_DARE_ENTID")
  local idmap_overridden = entid_env_raw ~= nil and entid_env_raw:match("^%s*{") ~= nil

  local env = runner.env_override({
    ["TRUTH_OR_DARE_TEST_DARE_ENTID"] = idmap,
    ["TRUTH_OR_DARE_TEST_LIVE"] = "FALSE",
    ["TRUTH_OR_DARE_TEST_EXPLAIN"] = "FALSE",
  })

  local idmap_resolved = helpers.to_map(
    env["TRUTH_OR_DARE_TEST_DARE_ENTID"])
