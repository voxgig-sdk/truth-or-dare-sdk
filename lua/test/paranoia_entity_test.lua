...
describe("ParanoiaEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Paranoia(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = paranoia_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "paranoia." .. _op, _live and "live" or "unit")
    ...
    local paranoia_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.paranoia")))
    local paranoia_ref01_data = nil
    if #paranoia_ref01_data_raw > 0 then
      paranoia_ref01_data = helpers.to_map(paranoia_ref01_data_raw[1][2])
    end

    -- LOAD
    local paranoia_ref01_ent = client:Paranoia(nil)
    local paranoia_ref01_match_dt0 = {
...
function paranoia_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/paranoia/ParanoiaTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read paranoia test data: " .. entity_data_file)
  end
