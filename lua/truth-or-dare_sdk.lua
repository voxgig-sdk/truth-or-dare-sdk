...
-- by passing it.
function TruthOrDareSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TruthOrDareSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "TruthOrDareSDK: graphql: " .. msg
    res.graphql = errors
  end
