...
class TestNhieEntity:

    def test_should_create_instance(self):
        testsdk = TruthOrDareSDK.test(None, None)
        ent = testsdk.Nhie(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _nhie_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "nhie." + _op, "live" if _live else "unit")
            if _skip:
...
def _nhie_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("TRUTH_OR_DARE_TEST_NHIE_ENTID"))
