...
from truthordare_sdk.core.spec import TruthOrDareSpec
from truthordare_sdk.core.result import TruthOrDareResult
from truthordare_sdk.core.response import TruthOrDareResponse
from truthordare_sdk.core.operation import TruthOrDareOperation
from truthordare_sdk.core.error import TruthOrDareError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return TruthOrDareError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = TruthOrDareSDK.test(None, None)
        ...
        class TestInitFeature(TruthOrDareBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(TruthOrDareBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = TruthOrDareSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = TruthOrDareSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = TruthOrDareOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = TruthOrDareSDK.test(None, setup_opts)
