...
class TruthOrDareTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TruthOrDareTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TruthOrDareTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TruthOrDareTestRunner.getenv("TRUTH_OR_DARE_TEST_LIVE")
        override = TruthOrDareTestRunner.getenv("TRUTH_OR_DARE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TruthOrDareTestRunner.getenv(key)
        ...
        explain = TruthOrDareTestRunner.getenv("TRUTH_OR_DARE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TRUTH_OR_DARE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TruthOrDareTestRunner._test_control is not None:
            return TruthOrDareTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TruthOrDareTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TruthOrDareTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TruthOrDareTestRunner.load_test_control()
...
def load_env_local():
    TruthOrDareTestRunner.load_env_local()


def env_override(m):
    return TruthOrDareTestRunner.env_override(m)


def entity_data(v):
    return TruthOrDareTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TruthOrDareTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TruthOrDareTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TruthOrDareTestRunner.load_test_control()


def live_client_options():
    return TruthOrDareTestRunner.live_client_options()


def live_delay_ms():
    return TruthOrDareTestRunner.live_delay_ms()
