...
The Python SDK for the TruthOrDare API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Dare(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.TruthOrDare(None).load(
    {"id": "test01"}, None
)
