...
from truthordare_sdk.truthordare_types import (
    Paranoia,
    ParanoiaLoadMatch,
)
...
class ParanoiaEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ParanoiaEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Paranoia:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Paranoia().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
